...
The evaluation of the _isFront_ property of a &#95;__SurfaceData__ object needs to take __gml:OrientableSurface__s into account, as those can flip the orientation of a surface. Assume a _gml:OrientableSurface_ _os_, which flips its base surface _bs_. A front side texture stem:[t] targeting _bs_ will appear on the actual front side of _bs_. If stem:[t] targets _os_, it will appear on the back side of _bs_. If stem:[t] targets both _os_ and _bs_, it appears on both sides of _bs_ since it becomes the front and back side texture.
